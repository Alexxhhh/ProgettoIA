(define (problem quest_problem)
  ...
  (:objects
    ...
    Spiky_Spine_Cavern Prickly_Pits Trolls_Treetops Arrow_Alley Flame_Forge - location
    Bubble_Bog Mushroom_Meadow Rainbow_Ridge Riddle_Reef Puzzle_Pond Enigma_Edge Mystery_Marsh Unicorns_Vault - location

    trap_spikes_1 trap_spikes_2 trap_arrow trap_fire trap_gorilla - trap
  ...
  (:init
    ...
    ;; Trappole
    (at trap_spikes_1 Spiky_Spine_Cavern)
    (at trap_spikes_2 Prickly_Pits)
    ...
    ;; Associazione chiavi → strade
    (key-for key1 strada24)
    (key-for key2 strada25)
    (key-for key3 strada26)
    (key-for key4 strada27)
  )
